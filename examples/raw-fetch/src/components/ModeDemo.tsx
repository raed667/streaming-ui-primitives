...
import { fromFetchSSE, type FetchSSEMode } from 'streaming-ui-primitives/adapters'
...
export function ModeDemo({ mode }: Props) {
  ...
  async function start() {
    ...
    setSource(
      mode === 'sse-json'
        ? fromFetchSSE(res, { mode: 'sse-json', jsonPath: 'choices.0.delta.content' })
  ...
  return (
    <div style={styles.card}>
      ...
      <StreamGuard
        ...
        idle={
          <p style={styles.idle}>
            Click <strong>Stream</strong> to fetch from the mock server using <code>{mode}</code> mode.
          </p>
        }
        streaming={
          <div>
            <StreamingText content={text} isStreaming cursor as="p" style={styles.output} />
            <p style={styles.hint}>Streaming… click Reset to cancel and clear.</p>
          </div>
        }
        complete={
          <div>
            <p style={styles.output}>{text}</p>
            <p style={styles.hint}>Complete. {text.length} characters received.</p>
          </div>
        }
        error={(err) => (
      ...
      <p style={styles.footNote}>
        <code>reset()</code> cancels the stream and clears accumulated text.
        Use <code>abort()</code> to stop without clearing.
      </p>
    </div>
  )
}
...
function modeSnippet(mode: FetchSSEMode): string {
  ...
  if (mode === 'sse-json') {
    return base + `const stream = fromFetchSSE(res, { mode: 'sse-json', jsonPath: 'choices.0.delta.content' })`
  }
  if (mode === 'auto') {
    return base + `const stream = fromFetchSSE(res) // mode: 'auto' is the default`
  }
...
const styles: Record<string, React.CSSProperties> = {
  ...
  codeBlock: { background: '#1e1e2e', borderRadius: 8, padding: '10px 14px', overflowX: 'auto' as const },
  code: { color: '#cdd6f4', fontSize: 13, whiteSpace: 'pre' as const, fontFamily: 'ui-monospace, monospace' },
  ...
  streamBtn: { padding: '7px 18px', background: '#1d4ed8', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 600, cursor: 'pointer' },
